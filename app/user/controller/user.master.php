...
class action extends app
{
	...
	private function modify()
	{
		...
		else
		{
			...
			foreach ($forms as $f => $fms) {
				if($fms['id'] == 'userprofile'){
					$forms[$f]['html'] = '<textarea class="form-control" name="args[userprofile]" id="userprofile">'.$user['userprofile'].'</textarea>';
				}
			}
	...
	private function userdetail()
	{
		...
		$where 		= array('ehuserid'=>$userid);
		
		// 用户考试记录考场ids
		$ehexamids 	= $arModel->where($where)
							  ->groupBy("ehexamid")
							  ->get("x2_examhistory",null,"ehexamid");
		$list 		= array();
		foreach ($ehexamids as $k => $v)
		{
			$where 		= array();
			$where 		= array('ehuserid'=>$userid,'ehexamid' => $v['ehexamid']);
		...
		$this->tpl->assign('user_score_list',$list);

		$this->tpl->display("userdetail");
	}
