...

{
...

	{
		...
		// $word->Visible = 0;
		// $word->Documents->open($content['course_files']);
	...
	public function click(){
		session_start();
		$id = $this->ev->get('id');
		$data = $this->db->mycount("select click from x2_course where courseid = ".$id);
		if($this->ev->get('type') == '1'){
			...
			$res = $this->db->mycount("update x2_course set click = ".$click." where courseid = ".$id);
			$_SESSION['clicked'] = '1';
			$message = "感谢您的支持";
			$type = '1';
		}elseif ($this->ev->get('type') == '0') {
			// if ($_SESSION['clicked'] == '0') {
			// 	$message = "您已经操作了哦";
			// 	$arr = array(
			// 		"statusCode" => 300,
			// 		"message" 	 => $message,
