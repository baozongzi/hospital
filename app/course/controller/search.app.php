...
class action extends app
{
	...
	private function index()
	{
		...
		foreach ($contents['data'] as $c => $con) {
			$contents['data'][$c]['click'] = "0";
			$result = $this->db->mycount("select click from x2_course where coursecsid = ".$con['csid']);
			foreach ($result as $r => $res) {
				$contents['data'][$c]['click'] += $res['click'];
			}
		}
