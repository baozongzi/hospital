...
class action extends app
{
	...
	private function selected()
	{
		$show = $this->ev->get('show');
			...
			foreach ($questions as $q => $qs) {
				foreach ($questions[$q]['questionknowsid'] as $qid => $qids) {
					$knowsid = $questions[$q]['questionknowsid'][$qid]['knowsid'];
					$knowsid = $questions[$q]['questionknowsid'] = unserialize($this->db->mycount('select qrknowsid  from x2_questionrows where qrid = '.$knowsid)[0]['qrknowsid']);
				}
			}
			$rowsquestions = array();
	...
	private function score()
	{
		...
		if (array_key_exists("search", $_REQUEST)) 
		{
			...
			if (!empty($stime) && !empty($etime)) {
				$this->tpl->assign('search',$search);
				$where .= ' and ehstarttime between '.strtotime($stime).' and '.strtotime($etime); 
			}
		}
		$list 		= array();
		...
		$list = $this->db->mycount('select * from x2_examhistory as eh join x2_user as u on eh.ehuserid = u.userid where ehexamid = '.$examid.' and '.$where.' order by ehstarttime desc');
		foreach ($list as $key => $value) {
			$list[$key]['passscore'] = unserialize($list[$key]['ehsetting'])['examsetting']['passscore'];
			// unset($list[$key]['ehsetting']);
		}
		$this->tpl->assign('examid',$examid);
