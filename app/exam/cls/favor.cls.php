...
class favor_exam
{
	...
	public function getExamScoreListByBasicId($basicid,$page,$uid)
	{
		$uid = $_SESSION['currentuser']['sessionuserid'];
		$data = array(
			'select' => false,
			'table' => array('examhistory','user'),
			'query' => array(array("AND","ehbasicid = $basicid"),array("AND","ehtype = 2"),array("AND","ehuserid = $uid"),array("AND","ehstatus = 1 group by ehid")),
