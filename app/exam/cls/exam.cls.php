...
class exam_exam
{
	...
	{
		$author = $_SESSION['currentuser']['sessionusername'];
			...
			if(count($question) >= 6)
			{
				...
				if($isqr)
				{
					...
					if($istitle)
					{
						...
						// if($tknowsid == "0"){
							$strs = htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[6]," \n\t")));
							$arrs = explode(',',$strs);
							$tknowsid = $arrs[0];
							$knows = $arrs[1];
							$questionknowsid['knowsid'] = $tknowsid;
							$questionknowsid['knows'] = $knows;
							// $data_qsid[] = $questionknowsid['knowsid'].':'.$questionknowsid['knows']."\n";
							$args['questionknowsid'] = $questionknowsid['knowsid'].':'.$questionknowsid['knows']."\n";
						// }else{
						// 	$args['questionknowsid'] = $tknowsid;
						// }
					...
					else
					{
						...
						// $args['questionusername'] = $this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[10]," \n\t"))));
						$args['questionusername'] = $author;
